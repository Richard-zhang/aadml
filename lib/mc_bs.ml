...
type model_dynamic_t = float expr -> float expr -> float expr

let bs_model_dynamic_2 ~rate ~vol : model_dynamic_t =
  let rand_gen = gen_normal_rv in
  fun delta_t value_at_t ->
...
let bs_model_dynamic ~rate ~vol : model_dynamic_t =
  let rand_gen = gen_coin_flip 1234 in
  fun delta_t value_at_t ->
...
let avg (values : float expr list) =
...
let single_underlying_mc ~(num_path : int) ~(num_step : int)
    ~(model_dynamic : model_dynamic_t)
...
*)
let mc_bs_impl num_path num_step ~vol ~stock ~strike ~t ~rate =
  let model_dynamic = bs_model_dynamic_2 ~rate ~vol in
  let pay_off_func = pay_off_func strike in
  single_underlying_mc ~num_path ~num_step ~model_dynamic ~pay_off_func
    ~init:stock ~r:rate ~time_in_year:t

let mc_bs = mc_bs_impl 10 3
